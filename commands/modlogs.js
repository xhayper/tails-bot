...
let modlogstatus = await db.fetch(`modlogs_${msg.guild.id}_stat`);
let modlogschannel = await db.fetch(`modlogs_${msg.guild.id}_channel`);
  
if (modlogstatus === null) var sstatus = 'off'
else var sstatus = modlogstatus;
if (modlogschannel === null) var dachannel = 'None'
else var dachannel = `<#${modlogschannel}>`;
  ...
  .setColor('RANDOM')
  .setDescription('modlogs on/off or modlogs channel {Mention A Channel}')
  .addField('Example', 'modlogs channel #example\nlogs on\nlogs off')
  .addField('Status', 'Mod Log : ' + status + "\n Current Mod Log Channel : " + dachannel)
...
//---------------------------------------------------------------------------------------------
  ...
  db.set(`modlogs_${msg.guild.id}_channel`, channels)
...
//---------------------------------------------------------------------------------------------
    ...
    const on = 'on';
    db.set(`modlogs_${msg.guild.id}_stat`, on)
...
//---------------------------------------------------------------------------------------------
    ...
    const off = 'off';
   db.set(`modlogs_${msg.guild.id}_stat`, off)
